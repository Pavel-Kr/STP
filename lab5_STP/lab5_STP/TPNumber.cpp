...
bool TPNumber::checkPrecision(string num_str, int precision)
{
	...
	for (char c : num_str) {
		...
		if (point_met)
			real_precision++;
		if (real_precision > precision && c != '0') return false;
	}
	return true;
}
...
{
	string num_str = toString(number);
	if (!checkStr(num_str)) return false;
	if (!checkBase(num_str, base)) return false;
	if (!checkPrecision(num_str, precision)) return false;
	return true;
}
...
TPNumber::TPNumber(string num_str, string base_str, string precision_str)
{
	try
	{
		...
		if (this->base < 17 && this->base > 1 && this->base != 10 && this->precision >= 0 && check(num_str, base_str, precision_str)) {
